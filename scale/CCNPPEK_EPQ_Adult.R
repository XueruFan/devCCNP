...
epqa <- rawdata[, c(1,2, 786:873)]
setwd(file.path(filefolder, "raw"))

# save a copy of raw data
write.xlsx(epqa, "CCNPPEK_EPQ_Adult_Batch1234_raw.xlsx", rowNames = F, colNames = F)
...
# 去掉没有做这个问卷的被试
epqa2 <- epqa[!apply(epqa[, (ncol(epqa)-3):ncol(epqa)], 1, function(x) all(is.na(x))), ]

epqa <- epqa2[, c(1,2,91:94)]

# save finial result of this scale
setwd(file.path(filefolder, "scale"))
write.xlsx(epqa, "CCNPPEK_EPQ_Adult_Batch1234.xlsx", rowNames = F, colNames = T)
