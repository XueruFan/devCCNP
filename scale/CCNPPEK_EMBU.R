...
embu <- rawdata[, c(1,2, 445:578)]
embu <- embu[-1,]

setwd(file.path(filefolder, "raw"))

# save a copy of raw data
write.xlsx(embu, "CCNPPEK_EMBU_Batch1234_raw.xlsx", rowNames = F, colNames = T)

# apply score according rules
embu_f <- embu[, c(1,2, seq(5, 135, by = 2))]
embu_m <- embu[, c(1,2, seq(6, 136, by = 2))]

colnames(embu_f)[3:68] <- seq(1, 66)
colnames(embu_m)[3:68] <- seq(1, 66)
...
write.xlsx(EMBU, "CCNPPEK_EMBU_Batch1234.xlsx", rowNames = F, colNames = T)
