...
rawfile <-  "CCNPPEK被试信息表_Batch1234.csv"
rawdata <- read.csv(rawfile)

############################# load handness file
setwd(paste0(filefolder, "/raw"))
hand_valid_file <-  "CCNPPEK_HandnessValid_Batch1234_raw.xlsx"
hand_valid <- read.xlsx(hand_valid_file)
hand_invalid_file <-  "CCNPPEK_HandnessInValid_Batch1234_raw.xlsx"
...
# 去掉没有年龄数据的被试
raw_ccnp <- raw_ccnp[!is.na(raw_ccnp$Age), ] 

# save a copy of raw data
write.xlsx(raw, "CCNPPEK_Demography_Batch1234_raw.xlsx", rowNames = F, colNames = T)

# save a version for open sharing
setwd(paste0(filefolder, "/scale"))
result <- raw_ccnp[, c(1,2,3,8,9)]
write.table(result, "CCNPPEK_Participant_Batch1234.tsv",  sep = "\t",  row.names = F, col.names = T, quote = F,)
