...
######################## Arrange EPQ Youth result ######################################################

epqy <- rawdata[, c(1,2, 278:365)]
setwd(file.path(filefolder, "raw"))

# save a copy of raw data
write.xlsx(epqy, "CCNPPEK_EPQ_Youth_Batch1234_raw.xlsx", rowNames = F, colNames = F)
...
# 反向计分题号
index <- c(30,66,77,9,49,4,11,16,20,40,67,70,73,80,82,87)
...
# 去掉没有做这个问卷的被试
epqy2 <- epqy[!apply(epqy[, (ncol(epqy)-3):ncol(epqy)], 1, function(x) all(is.na(x))), ]

epqy <- epqy2[, c(1,2,91:94)]


# save finial result of this scale
setwd(file.path(filefolder, "scale"))
write.xlsx(epqy, "CCNPPEK_EPQ_Youth_Batch1234.xlsx", rowNames = F, colNames = T)
