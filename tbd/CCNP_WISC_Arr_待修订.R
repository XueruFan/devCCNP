# 本代码用于把彩巢儿童青少年IQ报告的数据汇总成excel文件，以便进行后续的数据分析和报告制作
...
ReportsFil <- "F:/IQ原始报告/儿童青少年"
ResultFil <- "C:/系统文件/工作管理/项目管理/彩巢计划/数据相关/问卷录入/自录数据"
...
for (pdf_file in File_ID) {
  ...
  lines <- unlist(strsplit(table_text, "\n"))
  data_list <- lapply(lines, function(line) unlist(strsplit(trimws(line), " +")))
  Name$"实足年龄" <- paste0(data_list[[1]][2], "岁", data_list[[1]][3], "月", data_list[[1]][4], "日")
  ...
  lines <- unlist(strsplit(table_text, "\n"))
  data_list <- lapply(lines, function(line) unlist(strsplit(trimws(line), " +")))
  data_frame[4,2] <- c(data_list[[2]][1])
  
  pattern <- "矩阵推理[\\s\\S]+?理解"
  table_text <- regmatches(text, regexpr(pattern, text, perl = TRUE))
  lines <- unlist(strsplit(table_text, "\n"))
  data_list <- lapply(lines, function(line) unlist(strsplit(trimws(line), " +")))
  data_frame[8,2] <- c(data_list[[2]][1])
  ...
  pattern <- "量表分数总和: [\\s\\S]+?总智商: \\d+"
  # 量表分数总和: 匹配表格开始的文字
  # [\\s\\S]+? 匹配任意字符（包括换行），+?为非贪婪匹配，即尽可能少地匹配字符，直到遇到后面的限定条件
  # 总智商: \\d+ 匹配以“总智商: ”开头，后面跟着一串数字的文本，这里假设总智商的值由数字组成
  table_text <- regmatches(text, regexpr(pattern, text, perl = TRUE))
  lines <- unlist(strsplit(table_text, "\n"))
  data_list <- lapply(lines, function(line) unlist(strsplit(trimws(line), " +")))
  data_frame <- do.call(rbind.data.frame, data_list)
  Scale <- data.frame(data_frame[c(8,9,11,12,13), c(1:4)])
...
CCNP_IQ_Result$ID <- gsub("_w", "-W", CCNP_IQ_Result$ID)
colnames(CCNP_IQ_Result)[1] <- "FID"

setwd(ResultFil)
write.xlsx(CCNP_IQ_Result, "CCNPPEK_WISC_All.xlsx", rowNames = F, colNames = T)
